...
### [current] — сессия 2.2.0 (завершена)
Состояние: **релиз v1.7.5 выпущен** (404Brain-win32-x64-1.7.5.zip, tag v1.7.5, product 1.7.5/0054, коммит `3672cf82`; exe запекал HEAD `6c4ef90d`).

Сделано в этой сессии:
1. **Параллельные tool-вызовы**: модель может выдать НЕСКОЛЬКО `<tool>`-блоков в одном ответе; запускаются одновременно (`Promise.allSettled`), результаты возвращаются в LLM одним бэтчем (отдельный цикл агента).
   - `extractGrammar.ts`: мульти-парсер — каждый завершённый блок парсится строго до ПЕРВОЙ `</name>` (иначе два одноимённых вызова засоряли параметры друг друга); стабильные id `call_${toolName}_${idx}`; посимвольный стрим, шанки, сломанные `<tool_call>`-обёртки и «reactive-`>`` — всё работает (тест-харнесс `parsertest.js`, 7 кейсов зелёные).
   - `sendLLMMessageTypes.ts`: `OnText`/`OnFinalMessage` несут `toolCalls: RawToolCallObj[]`.
   - `chatThreadService.ts`: `llmDone.toolCalls`; батч: если хоть один вызов требует интерактивного одобрения → последовательно (UX Approve/Reject не ломаем), иначе параллельно; при abort посреди батча агент не «воскресает»; `_swapOutLatestStreamingToolWithResult` матчится по `id` (параллельные тулы не перезаписывают чужие тайлы).
   - Нативные пути (openai-style / anthropic / gemini) тоже отдают `toolCalls` (антропик собирает ВСЕ `tool_use`).
   - `prompts.ts`: `<tool_calling>` и ещё 2 места — «вы можете выдать НЕСКОЛЬКО независимых вызовов, КОНСЕКУТИВНО, без текста между ними, исполняются ПАРАЛЛЕЛЬНО» (строка «EXACT tool call format» сохранена для верификации бандла).
2. `npm run compile` ✅ (0 ошибок).
3. `product.json` → 1.7.5/0054. Desktop-сборка (11 мин), exe запекал HEAD `6c4ef90d` (проверено: product 1.7.5/0054, commit, парсер и «EXACT tool call format» + «execute in PARALLEL» в бандлах).
4. **REH-тар 1.99.3 перепатчен на `6c4ef90d…`** (локальная WSL-копия тара оказалась СТАРОЙ `3efe08f1`! Скачан с GitHub `brain-reh-linux-x64-1.99.3.tar.gz` (64092b2c, 52 537 791 B) → перепатч commit → перепаковка в WSL (exec-биты `-rwxr-xr-x`) → залит clobber (52 538 314 B). Урок: всегда перепроверять commit ВНУТРИ тара, локальная копия может устареть.
5. `.144`: патч commit `6c4ef90d…` в `~/.404brain-server/bin/{64092b2c,8dcfd2bc}*/product.json` (старые папки бинов заявляют новый commit).
6. Скан диффа на секреты ✅ (`rg sk-…|ghp_|api_key`), коммит `3672cf82`, push, `gh release create v1.7.5` + реф-ссылка `https://aitunnel.ru/?r=52512`.
7. Кейс: ревью сборки — если из папки `C:\Users\palapalaru\Desktop\VSCode-win32-x64` запущен `404Brain.exe`, `clean-vscode-win32-x64` падает `EBUSY`; перед сборкой закрыть инстансы.

### сессия 2.1.0 (завершена)
