...
export class LLMMessageService extends Disposable implements ILLMMessageService {
	...
	constructor(
		...
		// const service = ProxyChannel.toService<LLMMessageChannel>(mainProcessService.getChannel('brain-channel-sendLLMMessage')); // lets you call it like a service
		// see llmMessageChannel.ts
		this.channel = this.mainProcessService.getChannel('brain-channel-llmMessage')
	...
	sendLLMMessage(params: ServiceSendLLMMessageParams) {
		...
		if (modelSelection === null) {
			const message = `Please add a provider in 404Brain Settings.`
		...
		const { settingsOfProvider, } = this.brainSettingsService.state
