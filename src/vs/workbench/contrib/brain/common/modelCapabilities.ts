...
import { FeatureName, ModelSelectionOptions, OverridesOfModel, ProviderName } from './brainSettingsTypes.js';
...
export type BrainStaticModelInfo = { // not stateful
	// 404Brain uses the information below to know how to handle each model.
...
type BrainStaticProviderInfo = { // doesn't change (not stateful)
	...
	modelOptions: { [key: string]: BrainStaticModelInfo };
	modelOptionsFallback: (modelName: string, fallbackKnownValues?: Partial<BrainStaticModelInfo>) => (BrainStaticModelInfo & { modelName: string, recognizedModelName: string }) | null;
}
...
const defaultModelOptions = {
	...
	supportsFIM: false,
	reasoningCapabilities: false,
} as const satisfies BrainStaticModelInfo
...
} as const satisfies { [s: string]: Partial<BrainStaticModelInfo> }




// keep modelName, but use the fallback's defaults
const extensiveModelOptionsFallback: BrainStaticProviderInfo['modelOptionsFallback'] = (modelName, fallbackKnownValues) => {

	const lower = modelName.toLowerCase()

	const toFallback = <T extends { [s: string]: Omit<BrainStaticModelInfo, 'cost' | 'downloadable'> },>(obj: T, recognizedModelName: string & keyof T)
		: BrainStaticModelInfo & { modelName: string, recognizedModelName: string } => {
...
} as const satisfies { [s: string]: BrainStaticModelInfo }

const anthropicSettings: BrainStaticProviderInfo = {
...
const openAISettings: BrainStaticProviderInfo = {
...
const xAISettings: BrainStaticProviderInfo = {
...
const geminiSettings: BrainStaticProviderInfo = {
...
const deepseekSettings: BrainStaticProviderInfo = {
...
const mistralSettings: BrainStaticProviderInfo = {
...
const groqSettings: BrainStaticProviderInfo = {
...
} as const satisfies Record<string, BrainStaticModelInfo>
const googleVertexSettings: BrainStaticProviderInfo = {
...
const microsoftAzureSettings: BrainStaticProviderInfo = {
...
const awsBedrockSettings: BrainStaticProviderInfo = {
...
const vLLMSettings: BrainStaticProviderInfo = {
...
const lmStudioSettings: BrainStaticProviderInfo = {
...
const ollamaSettings: BrainStaticProviderInfo = {
	modelOptionsFallback: (modelName) => extensiveModelOptionsFallback(modelName, { downloadable: { sizeGb: 'not-known' } }),
...
const openaiCompatible: BrainStaticProviderInfo = {
...
const liteLLMSettings: BrainStaticProviderInfo = { // https://docs.litellm.ai/docs/reasoning_content
	modelOptionsFallback: (modelName) => extensiveModelOptionsFallback(modelName, { downloadable: { sizeGb: 'not-known' } }),
	modelOptions: {},
	providerReasoningIOSettings: {
		input: { includeInPayload: openAICompatIncludeInPayloadReasoning },
...
const openRouterSettings: BrainStaticProviderInfo = {
