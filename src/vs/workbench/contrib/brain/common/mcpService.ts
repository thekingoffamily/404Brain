...
import { MCPUserStateOfName } from './brainSettingsTypes.js';
...
class MCPService extends Disposable implements IMCPService {
	...
	constructor(
		...
		super();
		this.channel = this.mainProcessService.getChannel('brain-channel-mcp')
	...
	private async _initialize() {
		try {
			await this.brainSettingsService.waitForInitState;
	...
	private async _refreshMCPServers(): Promise<void> {
		...
		await this.brainSettingsService.addMCPUserStateOfNames(addedUserStateOfName);

		// delete isOn for any servers that no longer show up in the config
		await this.brainSettingsService.removeMCPUserStateOfNames(removedServerNames);
		...
		this.channel.call('refreshMCPServers', {
			...
			removedServerNames,
			updatedServerNames,
			userStateOfName: this.brainSettingsService.state.mcpUserStateOfName,
		})
	}
	...
	public async toggleServerIsOn(serverName: string, isOn: boolean): Promise<void> {
		...
		await this.brainSettingsService.setMCPServerState(serverName, { isOn });
