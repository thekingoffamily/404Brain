...
export type BrainSettingsState = {
...
// type RealBrainSettings = Exclude<keyof BrainSettingsState, '_modelOptions'>
// type EventProp<T extends RealBrainSettings = RealBrainSettings> = T extends 'globalSettings' ? [T, keyof BrainSettingsState[T]] : T | 'all'


export interface IBrainSettingsService {
	readonly _serviceBrand: undefined;
	readonly state: BrainSettingsState; // in order to play nicely with react, you should immutably change state
	...
	dangerousSetState(newState: BrainSettingsState): Promise<void>;
...
const _stateWithMergedDefaultModels = (state: BrainSettingsState): BrainSettingsState => {
...
const _validatedModelState = (state: Omit<BrainSettingsState, '_modelOptions'>): BrainSettingsState => {
...
export const IBrainSettingsService = createDecorator<IBrainSettingsService>('BrainSettingsService');
class BrainSettingsService extends Disposable implements IBrainSettingsService {
	_serviceBrand: undefined;

	private readonly _onDidChangeState = new Emitter<void>();
	readonly onDidChangeState: Event<void> = this._onDidChangeState.event; // this is primarily for use in react, so react can listen + update on state changes

	state: BrainSettingsState;
	...
	dangerousSetState = async (newState: BrainSettingsState) => {
	...
	private async _readState(): Promise<BrainSettingsState> {
		const encryptedState = this._storageService.get(BRAIN_SETTINGS_STORAGE_KEY, StorageScope.APPLICATION)
	...
	private async _storeState() {
		...
		this._storageService.store(BRAIN_SETTINGS_STORAGE_KEY, encryptedState, StorageScope.APPLICATION, StorageTarget.USER);
	}
	...
	setGlobalSetting: SetGlobalSettingFn = async (settingName, newVal) => {
		const newState: BrainSettingsState = {
			...this.state,
			globalSettings: {
	...
	toggleModelHidden(providerName: ProviderName, modelName: string) {
		...
		const newModels: BrainStatefulModelInfo[] = [
...
registerSingleton(IBrainSettingsService, BrainSettingsService, InstantiationType.Eager);
