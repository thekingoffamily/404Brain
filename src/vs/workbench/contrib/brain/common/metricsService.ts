...
export class MetricsService implements IMetricsService {
	...
	constructor(
		...
		this.metricsService = ProxyChannel.toService<IMetricsService>(mainProcessService.getChannel('brain-channel-metrics'));
	}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: 'brainDebugInfo',
			f1: true,
			title: localize2('brainMetricsDebug', '404Brain: Log Debug Info'),
		});
	}
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		notifService.info(`404Brain Debug info:\n${JSON.stringify(debugProperties, null, 2)}`)
	}
})
