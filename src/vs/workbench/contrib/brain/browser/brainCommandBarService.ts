...
import { mountBrainCommandBar } from './react/out/brain-editor-widgets-tsx/index.js'
...
import { BRAIN_ACCEPT_DIFF_ACTION_ID, BRAIN_REJECT_DIFF_ACTION_ID, BRAIN_GOTO_NEXT_DIFF_ACTION_ID, BRAIN_GOTO_PREV_DIFF_ACTION_ID, BRAIN_GOTO_NEXT_URI_ACTION_ID, BRAIN_GOTO_PREV_URI_ACTION_ID, BRAIN_ACCEPT_FILE_ACTION_ID, BRAIN_REJECT_FILE_ACTION_ID, BRAIN_ACCEPT_ALL_DIFFS_ACTION_ID, BRAIN_REJECT_ALL_DIFFS_ACTION_ID } from './actionIDs.js';
...
export interface IBrainCommandBarService {
...
export const IBrainCommandBarService = createDecorator<IBrainCommandBarService>('BrainCommandBarService');
...
export class BrainCommandBarService extends Disposable implements IBrainCommandBarService {
	_serviceBrand: undefined;

	static readonly ID: 'brain.BrainCommandBarService'
...
registerSingleton(IBrainCommandBarService, BrainCommandBarService, InstantiationType.Delayed); // delayed is needed here :(


export type BrainCommandBarProps = {
	uri: URI | null;
	editor: ICodeEditor;
}
...
class AcceptRejectAllFloatingWidget extends Widget implements IOverlayWidget {
	...
	constructor({ editor }: { editor: ICodeEditor, },
		...
		this.instantiationService.invokeFunction(accessor => {
			...
			const res = mountBrainCommandBar(root, accessor, { uri, editor } satisfies BrainCommandBarProps)
			if (!res) return
			this._register(toDisposable(() => res.dispose?.()))
			this._register(editor.onWillChangeModel((model) => {
				const uri = model.newModelUrl
				res.rerender({ uri, editor } satisfies BrainCommandBarProps)
			}))
		})
	}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: BRAIN_ACCEPT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('brainAcceptDiffAction', '404Brain: Accept Diff'),
			keybinding: {
				primary: KeyMod.CtrlCmd | KeyMod.Alt | KeyMod.Shift | KeyCode.Enter,
				mac: { primary: KeyMod.WinCtrl | KeyMod.Alt | KeyCode.Enter },
				weight: KeybindingWeight.BrainExtension,
			}
		});
	}
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		const commandBarService = accessor.get(IBrainCommandBarService);
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: BRAIN_REJECT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('brainRejectDiffAction', '404Brain: Reject Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: BRAIN_GOTO_NEXT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('brainGoToNextDiffAction', '404Brain: Go to Next Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: BRAIN_GOTO_PREV_DIFF_ACTION_ID,
			f1: true,
			title: localize2('brainGoToPrevDiffAction', '404Brain: Go to Previous Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: BRAIN_GOTO_NEXT_URI_ACTION_ID,
			f1: true,
			title: localize2('brainGoToNextUriAction', '404Brain: Go to Next File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: BRAIN_GOTO_PREV_URI_ACTION_ID,
			f1: true,
			title: localize2('brainGoToPrevUriAction', '404Brain: Go to Previous File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: BRAIN_ACCEPT_FILE_ACTION_ID,
			f1: true,
			title: localize2('brainAcceptFileAction', '404Brain: Accept All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: BRAIN_REJECT_FILE_ACTION_ID,
			f1: true,
			title: localize2('brainRejectFileAction', '404Brain: Reject All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: BRAIN_ACCEPT_ALL_DIFFS_ACTION_ID,
			f1: true,
			title: localize2('brainAcceptAllDiffsAction', '404Brain: Accept All Diffs in All Files'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: BRAIN_REJECT_ALL_DIFFS_ACTION_ID,
			f1: true,
			title: localize2('brainRejectAllDiffsAction', '404Brain: Reject All Diffs in All Files'),
			keybinding: {
