...
import { brainPrefixAndSuffix, ctrlKStream_userMessage, ctrlKStream_systemMessage, defaultQuickEditFimTags, rewriteCode_systemMessage, rewriteCode_userMessage, searchReplaceGivenDescription_systemMessage, searchReplaceGivenDescription_userMessage, tripleTick, } from '../common/prompt/prompts.js';
import { IBrainCommandBarService } from './brainCommandBarService.js';
import { IKeybindingService } from '../../../../platform/keybinding/common/keybinding.js';
import { BRAIN_ACCEPT_DIFF_ACTION_ID, BRAIN_REJECT_DIFF_ACTION_ID } from './actionIDs.js';
...
import { FeatureName } from '../common/brainSettingsTypes.js';
import { IBrainModelService } from '../common/brainModelService.js';
import { deepClone } from '../../../../base/common/objects.js';
import { acceptBg, acceptBorder, buttonFontSize, buttonTextColor, rejectBg, rejectBorder } from '../common/helpers/colors.js';
import { DiffArea, Diff, CtrlKZone, BrainFileSnapshot, DiffAreaSnapshotEntry, diffAreaSnapshotKeys, DiffZone, TrackingZone, ComputedDiff } from '../common/editCodeServiceTypes.js';
...
class EditCodeService extends Disposable implements IEditCodeService {
	...
	// 		message: `404Brain Error: ${e.message}`,
	// 		actions: {
	// 			secondary: [{
	// 				id: 'brain.onerror.opensettings',
	// 				enabled: true,
	// 				label: `Open 404Brain Settings`,
	// 				tooltip: '',
	// 				class: undefined,
	// 				run: () => { this._commandService.executeCommand(BRAIN_OPEN_SETTINGS_ACTION_ID) }
	// 			}]
	// 		},
	// 		source: details ? `(Hold ${isMacintosh ? 'Option' : 'Alt'} to hover) - ${details}\n\nIf this persists, feel free to [report](https://github.com/thekingoffamily/404Brain/issues/new) it.` : undefined
	// 	})
	// }
		...
		for (const diffareaid of this.diffAreasOfURI[uri.fsPath] || []) {
			...
			if (diffArea.type === 'DiffZone') {
				...
				if (diffArea._streamState.isStreaming) {
					// sweepLine ... sweepLine
					const fn1 = this._addLineDecoration(model, diffArea._streamState.line, diffArea._streamState.line, 'brain-sweepIdxBG')
					// sweepLine+1 ... endLine
					const fn2 = diffArea._streamState.line + 1 <= diffArea.endLine ?
						this._addLineDecoration(model, diffArea._streamState.line + 1, diffArea.endLine, 'brain-sweepBG')
						: null
				...
				const fn = this._addLineDecoration(model, diffArea.startLine, diffArea.endLine, 'brain-highlightBG')
	...
	private _addDiffStylesToURI = (uri: URI, diff: Diff) => {
		...
		if (type !== 'deletion') {
			const fn = this._addLineDecoration(model, diff.startLine, diff.endLine, 'brain-greenBG', {
		...
		if (diffZone.type === 'DiffZone' && !diffZone._streamState.isStreaming) {
			...
			const consistentWidgetId = this._consistentItemService.addConsistentItemToURI({
				...
				fn: (editor) => {
					...
					else { throw new Error('404Brain 1') }
	...
	private _getCurrentBrainFileSnapshot = (uri: URI): BrainFileSnapshot => {
		const { model } = this._voidModelService.getModel(uri)
	...
	private _restoreBrainFileSnapshot = async (uri: URI, snapshot: BrainFileSnapshot) => {
	...
	private _addToHistory(uri: URI, opts?: { onWillUndo?: () => void }) {
		const beforeSnapshot: BrainFileSnapshot = this._getCurrentBrainFileSnapshot(uri)
		let afterSnapshot: BrainFileSnapshot | null = null

		const elt: IUndoRedoElement = {
			type: UndoRedoElementType.Resource,
			resource: uri,
			label: '404Brain Agent',
			code: 'undoredo.editCode',
			undo: async () => { opts?.onWillUndo?.(); await this._restoreBrainFileSnapshot(uri, beforeSnapshot) },
			redo: async () => { if (afterSnapshot) await this._restoreBrainFileSnapshot(uri, afterSnapshot) }
		}
		this._undoRedoService.pushElement(elt)

		const onFinishEdit = async () => {
			afterSnapshot = this._getCurrentBrainFileSnapshot(uri)
	...
	public getBrainFileSnapshot(uri: URI) {
		return this._getCurrentBrainFileSnapshot(uri)
	}


	public restoreBrainFileSnapshot(uri: URI, snapshot: BrainFileSnapshot): void {
		this._restoreBrainFileSnapshot(uri, snapshot)
	}
	...
	private _writeStreamedDiffZoneLLMText(uri: URI, originalCode: string, llmTextSoFar: string, deltaText: string, latestMutable: StreamLocationMutable) {
		...
		if (!lastDiff) {
			...
			if (lastDiff.type === 'insertion' || lastDiff.type === 'edit')
				...
				throw new Error(`404Brain: diff.type not recognized on: ${lastDiff}`)
		}
	...
	private _initializeWriteoverStream(opts: StartApplyingOpts): [DiffZone, Promise<void>] | undefined {
		...
		if (from === 'ClickApply') {
			...
			throw new Error(`404Brain: diff.type not recognized on: ${from}`)
		}
		...
		if (from === 'ClickApply') {
			...
			const { prefix, suffix } = brainPrefixAndSuffix({ fullFileStr: originalFileCode, startLine, endLine })
			...
			while (shouldSendAnotherMessage) {
				...
				streamRequestIdRef.current = this._llmMessageService.sendLLMMessage({
					...
					onFinalMessage: async (params) => {
						...
						const blocks = extractSearchReplaceBlocks(fullText)
						if (blocks.length === 0) {
							this._notificationService.info(`404Brain: We ran Fast Apply, but the LLM didn't output any changes.`)
						}
						this._writeURIText(uri, originalFileCode, 'wholeFileRange', { shouldRealignDiffAreas: true })
	...
	public async acceptDiff({ diffid }: { diffid: number }) {
		...
		if (diff.type === 'deletion') {
			...
			throw new Error(`404Brain Error: ${diff}.type not recognized`)
		}
...
class AcceptRejectInlineWidget extends Widget implements IOverlayWidget {
	...
	constructor(
		...
		@IBrainCommandBarService private readonly _voidCommandBarService: IBrainCommandBarService,
		...
		const getAcceptRejectText = () => {
			const acceptKeybinding = this._keybindingService.lookupKeybinding(BRAIN_ACCEPT_DIFF_ACTION_ID);
			const rejectKeybinding = this._keybindingService.lookupKeybinding(BRAIN_REJECT_DIFF_ACTION_ID);
