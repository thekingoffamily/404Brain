...
import { IBrainSCMService } from '../common/brainSCMTypes.js'
import { IMainProcessService } from '../../../../platform/ipc/common/mainProcessService.js'
import { IBrainSettingsService } from '../common/brainSettingsService.js'
import { IConvertToLLMMessageService } from './convertToLLMMessageService.js'
import { ILLMMessageService } from '../common/sendLLMMessageService.js'
import { ModelSelection, OverridesOfModel, ModelSelectionOptions } from '../common/brainSettingsTypes.js'
...
export const IGenerateCommitMessageService = createDecorator<IGenerateCommitMessageService>('brainGenerateCommitMessageService');

const loadingContextKey = 'brainSCMGenerateCommitMessageLoading'
...
class GenerateCommitMessageService extends Disposable implements IGenerateCommitMessageService {
	...
	private brainSCM: IBrainSCMService
	...
	constructor(
		...
		this.brainSCM = ProxyChannel.toService<IBrainSCMService>(mainProcessService.getChannel('brain-channel-scm'))
	}
		...
		this.execute.trigger(async () => {
			...
			try {
				...
				const [stat, sampledDiffs, branch, log] = await Promise.all([
					this.brainSCM.gitStat(path),
					this.brainSCM.gitSampledDiffs(path),
					this.brainSCM.gitBranch(path),
					this.brainSCM.gitLog(path)
				])

				if (!this.isCurrentRequest(requestId)) { throw new CancellationError() }

				const modelSelection = this.brainSettingsService.state.modelSelectionOfFeature['SCM'] ?? null
				const modelSelectionOptions = modelSelection ? this.brainSettingsService.state.optionsOfModelSelection['SCM'][modelSelection?.providerName]?.[modelSelection.modelName] : undefined
				const overridesOfModel = this.brainSettingsService.state.overridesOfModel
	...
	private sendLLMMessage(messages: LLMChatMessage[], separateSystemMessage: string, modelOptions: ModelOptions): Promise<string> {
		return new Promise((resolve, reject) => {
			...
			this.llmRequestId = this.llmMessageService.sendLLMMessage({
				...
				logging: { loggingName: 'BrainSCM - Commit Message' },
			})
		})
	}
	...
	private onError(error: any) {
		if (!isCancellationError(error)) {
			console.error(error)
			this.notificationService.error(localize2('brainFailedToGenerateCommitMessage', 'Failed to generate commit message.').value)
		}
	}
}
...
class GenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			id: 'brain.generateCommitMessageAction',
			title: localize2('brainCommitMessagePrompt', '404Brain: Generate Commit Message'),
			icon: ThemeIcon.fromId('sparkle'),
			tooltip: localize2('brainCommitMessagePromptTooltip', '404Brain: Generate Commit Message'),
			f1: true,
			menu: [{
...
class LoadingGenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			id: 'brain.loadingGenerateCommitMessageAction',
			title: localize2('brainCommitMessagePromptCancel', '404Brain: Cancel Commit Message Generation'),
			icon: ThemeIcon.fromId('stop-circle'),
			tooltip: localize2('brainCommitMessagePromptCancelTooltip', '404Brain: Cancel Commit Message Generation'),
