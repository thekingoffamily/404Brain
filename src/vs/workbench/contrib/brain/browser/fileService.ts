...
class FilePromptActionService extends Action2 {
	private static readonly BRAIN_COPY_FILE_PROMPT_ID = 'brain.copyfileprompt'

	constructor() {
		super({
			id: FilePromptActionService.BRAIN_COPY_FILE_PROMPT_ID,
			title: localize2('brainCopyPrompt', '404Brain: Copy Prompt'),
			menu: [{
		...
		try {
			...
			if (stat.isFile) {
				m = await messageOfSelection({
					type: 'File',
					uri,
					language: (await brainModelService.getModelSafe(uri)).model?.getLanguageId() || '',
