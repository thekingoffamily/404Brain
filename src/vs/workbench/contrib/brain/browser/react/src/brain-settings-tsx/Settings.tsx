...
import { ProviderName, SettingName, displayInfoOfSettingName, providerNames, BrainStatefulModelInfo, customSettingNamesOfProvider, RefreshableProviderName, refreshableProviderNames, displayInfoOfProviderName, nonlocalProviderNames, localProviderNames, GlobalSettingName, featureNames, displayInfoOfFeatureName, isProviderNameDisabled, FeatureName, hasDownloadButtonsOnModelsProviderNames, subTextMdOfProviderName } from '../../../../common/brainSettingsTypes.js'
...
const ButtonLeftTextRightOption = ({ text, leftButton }: { text: string, leftButton?: React.ReactNode }) => {
	...
	return <div className='flex items-center text-brain-fg-3 px-3 py-0.5 rounded-sm overflow-hidden gap-2'>
...
const ConfirmButton = ({ children, onConfirm, className }: { children: React.ReactNode, onConfirm: () => void, className?: string }) => {
	...
	return (
		<div ref={ref} className={`inline-block`}>
			<BrainButtonBgDarken className={className} onClick={() => {
				if (!confirm) {
					setConfirm(true);
				} else {
					onConfirm();
...
const SimpleModelSettingsDialog = ({
	...
	const settingsStateService = accessor.get('IBrainSettingsService')
	...
	const sourcecodeOverridesLink = `https://github.com/thekingoffamily/404Brain/blob/main/src/vs/workbench/contrib/brain/common/modelCapabilities.ts#L146-L172`
	...
	return (
		...
		>
			{/* MODAL */}
			<div
				className="bg-brain-bg-1 rounded-md p-4 max-w-xl w-full shadow-xl overflow-y-auto max-h-[90vh]"
			...
			>
				<div className="flex justify-between items-center mb-4">
					...
					<button
						onClick={onClose}
						className="text-brain-fg-3 hover:text-brain-fg-1"
					>
						<X className="size-5" />
					</button>
				</div>
				...
				<div className="text-sm text-brain-fg-3 mb-4">
					{type === 'default' ? `${modelName} comes packaged with 404Brain, so you shouldn't need to change these settings.`
						: isUnrecognizedModel
							? `Model not recognized by 404Brain.`
							: `404Brain recognizes ${modelName} ("${recognizedModelName}").`}
				</div>


				{/* override toggle */}
				<div className="flex items-center gap-2 mb-4">
					<BrainSwitch size='xs' value={overrideEnabled} onChange={setOverrideEnabled} />
					<span className="text-brain-fg-3 text-sm">Override model defaults</span>
				</div>

				{/* Informational link */}
				{overrideEnabled && <div className="text-sm text-brain-fg-3 mb-4">
				...
				<textarea
					key={overrideEnabled + ''}
					ref={textAreaRef}
					className={`w-full min-h-[200px] p-2 rounded-sm border border-brain-border-2 bg-brain-bg-2 resize-none font-mono text-sm ${!overrideEnabled ? 'text-brain-fg-3' : ''}`}
				...
				<div className="flex justify-end gap-2 mt-4">
					<BrainButtonBgDarken onClick={onClose} className="px-3 py-1">
						Cancel
					</BrainButtonBgDarken>
					<BrainButtonBgDarken
						onClick={onSave}
...
export const ModelDump = ({ filteredProviders }: { filteredProviders?: ProviderName[] }) => {
	...
	const modelDump: (BrainStatefulModelInfo & { providerName: ProviderName, providerEnabled: boolean })[] = []
	...
	return <div className=''>
		{modelDump.map((m, i) => {
			...
			const detailAboutModel = type === 'autodetected' ?
				<Asterisk size={14} className="inline-block align-text-top brightness-115 stroke-[2] text-[#0e70c0]" data-tooltip-id='brain-tooltip' data-tooltip-place='right' data-tooltip-content='Detected locally' />
				: type === 'custom' ?
					<Asterisk size={14} className="inline-block align-text-top brightness-115 stroke-[2] text-[#0e70c0]" data-tooltip-id='brain-tooltip' data-tooltip-place='right' data-tooltip-content='Custom model' />
					: undefined
				...
				<div className="flex items-center gap-2 w-fit">
					...
					{disabled ? null : (
						<div className="w-5 flex items-center justify-center">
							...
							>
								<Plus size={12} className="text-brain-fg-3 opacity-50" />
							</button>
						</div>
					)}
					...
					<div className={`w-5 flex items-center justify-center`}>
						{type === 'default' || type === 'autodetected' ? null : <button
							...
							className={`${hasOverrides ? '' : 'opacity-0 group-hover:opacity-100'} transition-opacity`}
						>
							<X size={12} className="text-brain-fg-3 opacity-50" />
						</button>}
					</div>
				</div>
			</div>
		...
		{showCheckmark ? (
				...
				<form className="flex items-center gap-2">
					...
					<ErrorBoundary>
						<BrainCustomDropdownBox
							...
							className="max-w-32 mx-2 w-full resize-none bg-brain-bg-1 text-brain-fg-1 placeholder:text-brain-fg-3 border border-brain-border-2 focus:border-brain-border-1 py-1 px-2 rounded"
			...
			<div
				className="text-brain-fg-4 flex flex-nowrap text-nowrap items-center hover:brightness-110 cursor-pointer mt-4"
...
const ProviderSetting = ({ providerName, settingName, subTextMd }: { providerName: ProviderName, settingName: SettingName, subTextMd: React.ReactNode }) => {
	...
	const accessor = useAccessor()
	const brainSettingsService = accessor.get('IBrainSettingsService')
	const settingsState = useSettingsState()
	...
	const handleChangeValue = useCallback((newVal: string) => {
		brainSettingsService.setSettingOfProvider(providerName, settingName, newVal)
	}, [brainSettingsService, providerName, settingName]);
...
// 	const brainSettingsState = useSettingsState()

// 	const needsModel = isProviderNameDisabled(providerName, brainSettingsState) === 'addModel'

// 	// const accessor = useAccessor()
// 	// const brainSettingsService = accessor.get('IBrainSettingsService')

// 	// const { enabled } = brainSettingsState.settingsOfProvider[providerName]
...
// 				onChange={
// 					useCallback(() => {
// 						const enabledRef = brainSettingsService.state.settingsOfProvider[providerName].enabled
// 						brainSettingsService.setSettingOfProvider(providerName, 'enabled', !enabledRef)
// 					}, [brainSettingsService, providerName])}
// 				size='sm+'
// 			/> */}
// 		</div>
...
export const BrainProviderSettings = ({ providerNames }: { providerNames: ProviderName[] }) => {
	return <>
...
export const AutoDetectLocalModelsToggle = () => {
	...
	const enabled = brainSettingsState.globalSettings[settingName]

	return <ButtonLeftTextRightOption
		leftButton={<BrainSwitch
			size='xxs'
			value={enabled}
			onChange={(newVal) => {
				brainSettingsService.setGlobalSetting(settingName, newVal)
...
export const AIInstructionsBox = () => {
	...
	const brainSettingsService = accessor.get('IBrainSettingsService')
	const brainSettingsState = useSettingsState()
	return <BrainInputBox2
		className='min-h-[81px] p-3 rounded-sm'
		initValue={brainSettingsState.globalSettings.aiInstructions}
		...
		onChangeText={(newText) => {
			brainSettingsService.setGlobalSetting('aiInstructions', newText)
		}}
	/>
}
...
const FastApplyMethodDropdown = () => {
	...
	const onChangeOption = useCallback((newVal: boolean) => {
		brainSettingsService.setGlobalSetting('enableFastApply', newVal)
	}, [brainSettingsService])

	return <BrainCustomDropdownBox
		className='text-xs text-brain-fg-3 bg-brain-bg-1 border border-brain-border-1 rounded p-0.5 px-1'
		options={options}
		selectedOption={brainSettingsService.state.globalSettings.enableFastApply}
...
export const OllamaSetupInstructions = ({ sayWeAutoDetect }: { sayWeAutoDetect?: boolean }) => {
	return <div className='prose-p:my-0 prose-ol:list-decimal prose-p:py-0 prose-ol:my-0 prose-ol:py-0 prose-span:my-0 prose-span:py-0 text-brain-fg-3 text-sm list-decimal select-text'>
		<div className=''><ChatMarkdownRender string={`Ollama Setup Instructions`} chatMessageLocation={undefined} /></div>
		<div className=' pl-6'><ChatMarkdownRender string={`1. Download [Ollama](https://ollama.com/download).`} chatMessageLocation={undefined} /></div>
		<div className=' pl-6'><ChatMarkdownRender string={`2. Open your terminal.`} chatMessageLocation={undefined} /></div>
		<div
			className='pl-6 flex items-center w-fit'
			data-tooltip-id='brain-tooltip-ollama-settings'
		>
			<ChatMarkdownRender string={`3. Run \`ollama pull your_model\` to install a model.`} chatMessageLocation={undefined} />
		</div>
		{sayWeAutoDetect && <div className=' pl-6'><ChatMarkdownRender string={`404Brain automatically detects locally running models and enables them.`} chatMessageLocation={undefined} /></div>}
	</div>
}
...
const RedoOnboardingButton = ({ className }: { className?: string }) => {
	...
	const brainSettingsService = accessor.get('IBrainSettingsService')
	return <div
		className={`text-brain-fg-4 flex flex-nowrap text-nowrap items-center hover:brightness-110 cursor-pointer ${className}`}
		onClick={() => { brainSettingsService.setGlobalSetting('isOnboardingComplete', false) }}
	>
...
export const ToolApprovalTypeSwitch = ({ approvalType, size, desc }: { approvalType: ToolApprovalType, size: "xxs" | "xs" | "sm" | "sm+" | "md", desc: string }) => {
	...
	const onToggleAutoApprove = useCallback((approvalType: ToolApprovalType, newValue: boolean) => {
		brainSettingsService.setGlobalSetting('autoApprove', {
			...brainSettingsService.state.globalSettings.autoApprove,
			[approvalType]: newValue
		})
		metricsService.capture('Tool Auto-Accept Toggle', { enabled: newValue })
	}, [brainSettingsService, metricsService])

	return <>
		<BrainSwitch
			size={size}
			value={brainSettingsState.globalSettings.autoApprove[approvalType] ?? false}
			onChange={(newVal) => onToggleAutoApprove(approvalType, newVal)}
		/>
		<span className="text-brain-fg-3 text-xs">{desc}</span>
	</>
}
...
export const OneClickSwitchButton = ({ fromEditor = 'VS Code', className = '' }: { fromEditor?: TransferEditorType, className?: string }) => {
	...
	return <>
		<BrainButtonBgDarken className={`max-w-48 p-4 ${className}`} disabled={transferState.type !== 'done'} onClick={onClick}>
...
const MCPServerComponent = ({ name, server }: { name: string, server: MCPServer }) => {
	...
	const brainSettings = useSettingsState()
	const isOn = brainSettings.mcpUserStateOfName[name]?.isOn

	const removeUniquePrefix = (name: string) => name.split('_').slice(1).join('_')

	return (
		<div className="border border-brain-border-2 bg-brain-bg-1 py-3 px-4 rounded-sm my-2">
						...
						${server.status === 'success' ? 'bg-green-500'
							: server.status === 'error' ? 'bg-red-500'
								: server.status === 'loading' ? 'bg-yellow-500'
									: server.status === 'offline' ? 'bg-brain-fg-3'
										: ''}
					`}></div>

					{/* Server name */}
					<div className="text-sm font-medium text-brain-fg-1">{name}</div>
				</div>
					...
					<div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto">
						{(server.tools ?? []).length > 0 ? (
							(server.tools ?? []).map((tool: { name: string; description?: string }) => (
								<span
									key={tool.name}
									className="px-2 py-0.5 bg-brain-bg-2 text-brain-fg-3 rounded-sm text-xs"

									data-tooltip-id='brain-tooltip'
									data-tooltip-content={tool.description || ''}
									data-tooltip-class-name='brain-max-w-[300px]'
								>
									{removeUniquePrefix(tool.name)}
								</span>
							))
						) : (
							<span className="text-xs text-brain-fg-3">No tools available</span>
						)}
					</div>
				</div>
			)}
			...
			{isOn && server.command && (
				<div className="mt-3">
					<div className="text-xs text-brain-fg-3 mb-1">Command:</div>
					<div className="px-2 py-1 bg-brain-bg-2 text-xs font-mono overflow-x-auto whitespace-nowrap text-brain-fg-2 rounded-sm">
						{server.command}
					</div>
				</div>
			)}
...
const MCPServersList = () => {
	...
	if (mcpServiceState.error) {
		content = <div className="text-brain-fg-3 text-sm mt-2">
...
export const Settings = () => {
	...
	const onDownload = (t: 'Chats' | 'Settings') => {
		...
		if (t === 'Chats') {
			...
			downloadName = 'brain-chats.json'
		}
		else if (t === 'Settings') {
			// Export user settings
			dataStr = JSON.stringify(brainSettingsService.state, null, 2)
			downloadName = 'brain-settings.json'
		}
		else {
			dataStr = ''
			downloadName = ''
		...
		reader.onload = () => {
			try {
				...
				if (t === 'Chats') {
					...
					brainSettingsService.dangerousSetState(json as any)
				}
	...
	return (
		<div className={`@@brain-scope ${isDark ? 'dark' : ''}`} style={{ height: '100%', width: '100%', overflow: 'auto' }}>
          ...
          ${selectedSection === tab
										...
										: 'bg-brain-bg-2 hover:bg-brain-bg-2/80 text-brain-fg-1'}
        `}
							>
								{label}
							</button>
				...
				<main className="flex-1 p-6 select-none">
					...
					<div className='max-w-3xl'>
						...
						<h1 className='text-2xl w-full'>{`404Brain Settings`}</h1>
						...
						<div className='flex flex-col gap-12'>
							...
							<div className={shouldShowTab('localProviders') ? `` : 'hidden'}>
								<ErrorBoundary>
									...
									<h3 className={`text-brain-fg-3 mb-2`}>{`404Brain can access any model that you host locally. We automatically detect your local models by default.`}</h3>

									<div className='opacity-80 mb-4'>
										<OllamaSetupInstructions sayWeAutoDetect={true} />
									</div>

									<BrainProviderSettings providerNames={localProviderNames} />
								</ErrorBoundary>
							</div>
							...
							<div className={shouldShowTab('providers') ? `` : 'hidden'}>
								<ErrorBoundary>
									...
									<h3 className={`text-brain-fg-3 mb-2`}>{`404Brain can access models from Anthropic, OpenAI, OpenRouter, and more.`}</h3>

									<BrainProviderSettings providerNames={nonlocalProviderNames} />
								</ErrorBoundary>
							</div>
									...
									<div className='flex flex-col gap-y-8 my-4'>
										<ErrorBoundary>
											...
											<div>
												...
												<div className='text-sm text-brain-fg-3 mt-1'>
													<span>
														Experimental.{' '}
													</span>
													<span
														...
														data-tooltip-class-name='brain-max-w-[20px]'
													>
												...
												<div className='my-2'>
													...
													<ErrorBoundary>
														<div className='flex items-center gap-x-2 my-2'>
															<BrainSwitch
																...
																onChange={(newVal) => brainSettingsService.setGlobalSetting('enableAutocomplete', newVal)}
															/>
															<span className='text-brain-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.enableAutocomplete ? 'Enabled' : 'Disabled'}</span>
														</div>
													</ErrorBoundary>
													...
													<ErrorBoundary>
														<div className={`my-2 ${!settingsState.globalSettings.enableAutocomplete ? 'hidden' : ''}`}>
															<ModelDropdown featureName={'Autocomplete'} className='text-xs text-brain-fg-3 bg-brain-bg-1 border border-brain-border-1 rounded p-0.5 px-1' />
														</div>
													</ErrorBoundary>
										...
										<ErrorBoundary>
											...
											<div className='w-full'>
												...
												<div className='text-sm text-brain-fg-3 mt-1'>Settings that control the behavior of the Apply button.</div>
												...
												<div className='my-2'>
													...
													<div className='flex items-center gap-x-2 my-2'>
														<BrainSwitch
															...
															onChange={(newVal) => brainSettingsService.setGlobalSetting('syncApplyToChat', newVal)}
														/>
														<span className='text-brain-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.syncApplyToChat ? 'Same as Chat model' : 'Different model'}</span>
													</div>

													{/* Model Dropdown */}
													<div className={`my-2 ${settingsState.globalSettings.syncApplyToChat ? 'hidden' : ''}`}>
														<ModelDropdown featureName={'Apply'} className='text-xs text-brain-fg-3 bg-brain-bg-1 border border-brain-border-1 rounded p-0.5 px-1' />
													</div>
												</div>
										...
										<div>
											...
											<div className='text-sm text-brain-fg-3 mt-1'>{`Tools are functions that LLMs can call. Some tools require user approval.`}</div>
												...
												<ErrorBoundary>
													...
													<div className='flex items-center gap-x-2 my-2'>
														<BrainSwitch
															...
															onChange={(newVal) => brainSettingsService.setGlobalSetting('includeToolLintErrors', newVal)}
														/>
														<span className='text-brain-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.includeToolLintErrors ? 'Fix lint errors' : `Fix lint errors`}</span>
													</div>
												</ErrorBoundary>
												...
												<ErrorBoundary>
													<div className='flex items-center gap-x-2 my-2'>
														<BrainSwitch
															...
															onChange={(newVal) => brainSettingsService.setGlobalSetting('autoAcceptLLMChanges', newVal)}
														/>
														<span className='text-brain-fg-3 text-xs pointer-events-none'>Auto-accept LLM changes</span>
													</div>
												</ErrorBoundary>
											</div>
										</div>
										...
										<div className='w-full'>
											...
											<div className='text-sm text-brain-fg-3 mt-1'>{`Settings that control the visibility of 404Brain suggestions in the code editor.`}</div>
												...
												<ErrorBoundary>
													<div className='flex items-center gap-x-2 my-2'>
														<BrainSwitch
															...
															onChange={(newVal) => brainSettingsService.setGlobalSetting('showInlineSuggestions', newVal)}
														/>
														<span className='text-brain-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.showInlineSuggestions ? 'Show suggestions on select' : 'Show suggestions on select'}</span>
													</div>
												</ErrorBoundary>
											</div>
										</div>
										...
										<ErrorBoundary>
											...
											<div className='w-full'>
												...
												<div className='text-sm text-brain-fg-3 mt-1'>Settings that control the behavior of the commit message generator.</div>
												...
												<div className='my-2'>
													...
													<div className='flex items-center gap-x-2 my-2'>
														<BrainSwitch
															...
															onChange={(newVal) => brainSettingsService.setGlobalSetting('syncSCMToChat', newVal)}
														/>
														<span className='text-brain-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.syncSCMToChat ? 'Same as Chat model' : 'Different model'}</span>
													</div>

													{/* Model Dropdown */}
													<div className={`my-2 ${settingsState.globalSettings.syncSCMToChat ? 'hidden' : ''}`}>
														<ModelDropdown featureName={'SCM'} className='text-xs text-brain-fg-3 bg-brain-bg-1 border border-brain-border-1 rounded p-0.5 px-1' />
													</div>
												</div>
							...
							<div className={`${shouldShowTab('general') ? `` : 'hidden'} flex flex-col gap-12`}>
								...
								<div>
									<ErrorBoundary>
										...
										<h4 className='text-brain-fg-3 mb-4'>{`Transfer your editor settings into 404Brain.`}</h4>
								...
								<div>
									...
									<h4 className='text-brain-fg-3 mb-4'>{`Transfer 404Brain Settings and chats in and out of 404Brain.`}</h4>
									<div className='flex flex-col gap-8'>
										{/* Settings Subcategory */}
										<div className='flex flex-col gap-2 max-w-48 w-full'>
											<input key={2 * s} ref={fileInputSettingsRef} type='file' accept='.json' className='hidden' onChange={handleUpload('Settings')} />
											<BrainButtonBgDarken className='px-4 py-1 w-full' onClick={() => { fileInputSettingsRef.current?.click() }}>
												Import Settings
											</BrainButtonBgDarken>
											<BrainButtonBgDarken className='px-4 py-1 w-full' onClick={() => onDownload('Settings')}>
												Export Settings
											</BrainButtonBgDarken>
											<ConfirmButton className='px-4 py-1 w-full' onConfirm={() => { brainSettingsService.resetState(); }}>
												Reset Settings
											</ConfirmButton>
										</div>
										...
										<div className='flex flex-col gap-2 max-w-48 w-full'>
											...
											<BrainButtonBgDarken className='px-4 py-1 w-full' onClick={() => { fileInputChatsRef.current?.click() }}>
												Import Chats
											</BrainButtonBgDarken>
											<BrainButtonBgDarken className='px-4 py-1 w-full' onClick={() => onDownload('Chats')}>
												Export Chats
											</BrainButtonBgDarken>
								...
								<div>
									...
									<h4 className={`text-brain-fg-3 mb-4`}>{`IDE settings, keyboard settings, and theme customization.`}</h4>

									<ErrorBoundary>
										<div className='flex flex-col gap-2 justify-center max-w-48 w-full'>
											<BrainButtonBgDarken className='px-4 py-1' onClick={() => { commandService.executeCommand('workbench.action.openSettings') }}>
												General Settings
											</BrainButtonBgDarken>
											<BrainButtonBgDarken className='px-4 py-1' onClick={() => { commandService.executeCommand('workbench.action.openGlobalKeybindings') }}>
												Keyboard Settings
											</BrainButtonBgDarken>
											<BrainButtonBgDarken className='px-4 py-1' onClick={() => { commandService.executeCommand('workbench.action.selectTheme') }}>
												Theme Settings
											</BrainButtonBgDarken>
											<BrainButtonBgDarken className='px-4 py-1' onClick={() => { nativeHostService.showItemInFolder(environmentService.logsHome.fsPath) }}>
												Open Logs
											</BrainButtonBgDarken>
										</div>
									</ErrorBoundary>
								...
								<div className='max-w-[600px]'>
									...
									<h4 className={`text-brain-fg-3 mb-4`}>Very basic anonymous usage tracking helps us Keep 404Brain running smoothly. You may opt out below. Regardless of this setting, 404Brain never sees your code, messages, or API keys.</h4>
									...
									<div className='my-2'>
										...
										<ErrorBoundary>
											<div className='flex items-center gap-x-2 my-2'>
												...
												/>
												<span className='text-brain-fg-3 text-xs pointer-events-none'>{'Opt-out (requires restart)'}</span>
											</div>
										</ErrorBoundary>
									</div>
								</div>
										...
										<ErrorBoundary>
											<div className='flex items-center gap-x-2'>
												<BrainSwitch
													...
													onChange={(newValue) => {
														brainSettingsService.setGlobalSetting('disableSystemMessage', newValue);
													}}
												/>
												<span className='text-brain-fg-3 text-xs pointer-events-none'>
													{'Disable system message'}
												</span>
											</div>
										</ErrorBoundary>
										<div className='text-brain-fg-3 text-xs mt-1'>
											{`When disabled, 404Brain will not include anything in the system message except for content you specified above.`}
										</div>
									</div>
								</div>
...
Use Model Context Protocol to provide Agent mode with more tools.
							`} chatMessageLocation={undefined} />
									...
									<div className='my-2'>
										<BrainButtonBgDarken className='px-4 py-1 w-full max-w-48' onClick={async () => { await mcpService.revealMCPConfigFile() }}>
											Add MCP Server
										</BrainButtonBgDarken>
									</div>
