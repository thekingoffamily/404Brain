...
import { BrainCommandBarProps } from '../../../brainCommandBarService.js';
import { Check, EllipsisVertical, Menu, MoveDown, MoveLeft, MoveRight, MoveUp, X } from 'lucide-react';
import {
	BRAIN_GOTO_NEXT_DIFF_ACTION_ID,
	BRAIN_GOTO_PREV_DIFF_ACTION_ID,
	BRAIN_GOTO_NEXT_URI_ACTION_ID,
	BRAIN_GOTO_PREV_URI_ACTION_ID,
	BRAIN_ACCEPT_FILE_ACTION_ID,
	BRAIN_REJECT_FILE_ACTION_ID,
	BRAIN_ACCEPT_ALL_DIFFS_ACTION_ID,
	BRAIN_REJECT_ALL_DIFFS_ACTION_ID
} from '../../../actionIDs.js';

export const BrainCommandBarMain = ({ uri, editor }: BrainCommandBarProps) => {
	const isDark = useIsDark()

	return <div
		className={`@@brain-scope ${isDark ? 'dark' : ''}`}
	>
		<BrainCommandBar uri={uri} editor={editor} />
	</div>
}
...
export const BrainCommandBar = ({ uri, editor }: BrainCommandBarProps) => {
	...
	const _upKeybinding = keybindingService.lookupKeybinding(BRAIN_GOTO_PREV_DIFF_ACTION_ID);
	const _downKeybinding = keybindingService.lookupKeybinding(BRAIN_GOTO_NEXT_DIFF_ACTION_ID);
	const _leftKeybinding = keybindingService.lookupKeybinding(BRAIN_GOTO_PREV_URI_ACTION_ID);
	const _rightKeybinding = keybindingService.lookupKeybinding(BRAIN_GOTO_NEXT_URI_ACTION_ID);
	const _acceptFileKeybinding = keybindingService.lookupKeybinding(BRAIN_ACCEPT_FILE_ACTION_ID);
	const _rejectFileKeybinding = keybindingService.lookupKeybinding(BRAIN_REJECT_FILE_ACTION_ID);
	const _acceptAllKeybinding = keybindingService.lookupKeybinding(BRAIN_ACCEPT_ALL_DIFFS_ACTION_ID);
	const _rejectAllKeybinding = keybindingService.lookupKeybinding(BRAIN_REJECT_ALL_DIFFS_ACTION_ID);
	...
	if (currFileIdx === null) {
		return (
			<div className="pointer-events-auto">
				<div className="flex bg-brain-bg-2 shadow-md border border-brain-border-2 [&>*:first-child]:pl-3 [&>*:last-child]:pr-3 [&>*]:border-r [&>*]:border-brain-border-2 [&>*:last-child]:border-r-0">
		...
		<div className="pointer-events-auto">
			...
			{showAcceptRejectAllButtons && showAcceptRejectAll && (
				<div className="flex justify-end mb-1">
					<div className="inline-flex bg-brain-bg-2 rounded shadow-md border border-brain-border-2 overflow-hidden">
						<div className="flex items-center [&>*]:border-r [&>*]:border-brain-border-2 [&>*:last-child]:border-r-0">
			...
			<div className="flex items-center bg-brain-bg-2 rounded shadow-md border border-brain-border-2 [&>*:first-child]:pl-3 [&>*:last-child]:pr-3 [&>*]:px-3 [&>*]:border-r [&>*]:border-brain-border-2 [&>*:last-child]:border-r-0">
