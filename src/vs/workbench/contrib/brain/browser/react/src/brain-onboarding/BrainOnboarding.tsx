...
export const BrainOnboarding = () => {

	const brainSettingsState = useSettingsState()
	const isOnboardingComplete = brainSettingsState.globalSettings.isOnboardingComplete || OVERRIDE_VALUE

	const isDark = useIsDark()

	return (
		<div className={`@@brain-scope ${isDark ? 'dark' : ''}`}>
			<div
				className={`
					bg-brain-bg-3 fixed top-0 right-0 bottom-0 left-0 width-full z-[99999]
...
const AddProvidersPage = ({ pageIndex, setPageIndex }: { pageIndex: number, setPageIndex: (index: number) => void }) => {
	...
	return (<div className="flex flex-col md:flex-row w-full h-[80vh] gap-6 max-w-[900px] mx-auto relative">
		{/* Left Column */}
		<div className="md:w-1/4 w-full flex flex-col gap-6 p-6 border-none border-brain-border-2 h-full overflow-y-auto">
			{/* Tab Selector */}
		...
		<div className="flex-1 flex flex-col items-center justify-start p-6 h-full overflow-y-auto">
			...
			<div className="w-full max-w-xl mt-4 mb-10">
				...
				<div className="text-sm opacity-80 text-brain-fg-3 my-4 w-full">{descriptionOfTab[currentTab]}</div>
			</div>
			...
			{(currentTab === 'Local' || currentTab === 'Cloud/Other') && (
				<div className="w-full max-w-xl mt-8 bg-brain-bg-2/50 rounded-lg p-6 border border-brain-border-4">
					<div className="flex items-center gap-2 mb-4">
						<div className="text-xl font-medium">Models</div>
					</div>

					{currentTab === 'Local' && (
						<div className="text-sm opacity-80 text-brain-fg-3 my-4 w-full">Local models should be detected automatically. You can add custom models below.</div>
					)}
...
// 		title:
// 			div
// 				"How would you like to use Brain?"
// 		content:
...
const PreviousButton = ({ onClick, ...props }: { onClick: () => void } & React.ButtonHTMLAttributes<HTMLButtonElement>) => {
	return (
		<button
			onClick={onClick}
			className="px-6 py-2 rounded text-brain-fg-3 opacity-80 hover:brightness-115 duration-600 transition-all"
			{...props}
		>
			Back
		</button>
...
const OllamaDownloadOrRemoveModelButton = ({ modelName, isModelInstalled, sizeGb }: { modelName: string, isModelInstalled: boolean, sizeGb: number | false | 'not-known' }) => {
	...
	return <a
		...
		className="flex items-center justify-center text-brain-fg-2 hover:text-brain-fg-1"
	>
...
const BrainOnboardingContent = () => {


	const accessor = useAccessor()
	const brainSettingsService = accessor.get('IBrainSettingsService')
	const brainMetricsService = accessor.get('IMetricsService')
	...
	const didFillInProviderSettings = selectedProviderName && brainSettingsState.settingsOfProvider[selectedProviderName]._didFillInProviderSettings
	const isApiKeyLongEnoughIfApiKeyExists = selectedProviderName && brainSettingsState.settingsOfProvider[selectedProviderName].apiKey ? brainSettingsState.settingsOfProvider[selectedProviderName].apiKey.length > 15 : true
	const isAtLeastOneModel = selectedProviderName && brainSettingsState.settingsOfProvider[selectedProviderName].models.length >= 1
	...
	const lastPagePrevAndNextButtons = <div className="max-w-[600px] w-full mx-auto flex flex-col items-end">
		<div className="flex items-center gap-2">
			...
			<PrimaryActionButton
				onClick={() => {
					brainSettingsService.setGlobalSetting('isOnboardingComplete', true);
					brainMetricsService.capture('Completed Onboarding', { selectedProviderName, wantToUseOption })
				}}
				ringSize={brainSettingsState.globalSettings.isOnboardingComplete ? 'screen' : undefined}
			>Enter the Brain</PrimaryActionButton>
		</div>
	</div>
	...
	useEffect(() => {
		if (!brainSettingsState.globalSettings.isOnboardingComplete) {
			setPageIndex(0)
		}
	}, [setPageIndex, brainSettingsState.globalSettings.isOnboardingComplete])
			...
			content={
				<div>
					...
					<div className="mt-8 text-center flex flex-col items-center gap-4 w-full max-w-md mx-auto">
						<h4 className="text-brain-fg-3 mb-4">Transfer your settings from an existing editor?</h4>
