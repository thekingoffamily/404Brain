...
import { getBasename, getRelative, brainOpenFileFn } from '../sidebar-tsx/SidebarChat.js'
...
const Codespan = ({ text, className, onClick, tooltip }: { text: string, className?: string, onClick?: () => void, tooltip?: string }) => {
	...
	return <code
		className={`font-mono font-medium rounded-sm bg-brain-bg-1 px-1 ${className}`}
		onClick={onClick}
		{...tooltip ? {
...
const CodespanWithLink = ({ text, rawText, chatMessageLocation }: { text: string, rawText: string, chatMessageLocation: ChatMessageLocation }) => {
	...
	const onClick = () => {
		if (!link) return;
		// Use the updated brainOpenFileFn to open the file and handle selection
		if (link.selection)
			brainOpenFileFn(link.uri, accessor, [link.selection.startLineNumber, link.selection.endLineNumber]);
		else
			brainOpenFileFn(link.uri, accessor);
	}
...
const RenderToken = ({ token, inPTag, codeURI, chatMessageLocation, tokenIdx, ...options }: { token: Token | string, inPTag?: boolean, codeURI?: URI, chatMessageLocation?: ChatMessageLocation, tokenIdx: string, } & RenderTokenOptions): React.ReactNode => {
	...
	if (t.type === 'table') {
		...
		// return (
		// 	<div>
		// 		<table className={'min-w-full border border-brain-bg-2'}>
		// 			<thead>
		// 				<tr className='bg-brain-bg-1'>
		// 					{t.header.map((cell: any, index: number) => (
		// 						<th
		// 							key={index}
		// 							className='px-4 py-2 border border-brain-bg-2 font-semibold'
		...
		// 					<tr key={rowIndex} className={rowIndex % 2 === 0 ? 'bg-white' : 'bg-brain-bg-1'}>
		// 						{row.map((cell: any, cellIndex: number) => (
		// 							<td
		// 								key={cellIndex}
		// 								className={'px-4 py-2 border border-brain-bg-2'}
