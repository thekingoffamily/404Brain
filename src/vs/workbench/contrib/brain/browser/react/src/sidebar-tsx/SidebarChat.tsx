...
import { ToolApprovalTypeSwitch } from '../brain-settings-tsx/Settings.js';
...
const ReasoningOptionSlider = ({ featureName }: { featureName: FeatureName }) => {
	...
	const modelSelection = brainSettingsState.modelSelectionOfFeature[featureName]
	const overridesOfModel = brainSettingsState.overridesOfModel
	...
	const modelSelectionOptions = brainSettingsState.optionsOfModelSelection[featureName][providerName]?.[modelName]
	...
	if (canTurnOffReasoning && !reasoningBudgetSlider) { // if it's just a on/off toggle without a power slider
		return <div className='flex items-center gap-x-2'>
			<span className='text-brain-fg-3 text-xs pointer-events-none inline-block w-10 pr-1'>Thinking</span>
			<BrainSwitch
				size='xxs'
				value={isReasoningEnabled}
				onChange={(newVal) => {
					const isOff = canTurnOffReasoning && !newVal
					brainSettingsService.setOptionsOfModelSelection(featureName, modelSelection.providerName, modelSelection.modelName, { reasoningEnabled: !isOff })
				}}
			/>
		</div>
	}
	...
	if (reasoningBudgetSlider?.type === 'budget_slider') { // if it's a slider
		...
		const value = isReasoningEnabled ? brainSettingsState.optionsOfModelSelection[featureName][modelSelection.providerName]?.[modelSelection.modelName]?.reasoningBudget ?? defaultVal
			: valueIfOff
		...
		return <div className='flex items-center gap-x-2'>
			...
			<BrainSlider
				...
				onChange={(newVal) => {
					...
					brainSettingsService.setOptionsOfModelSelection(featureName, modelSelection.providerName, modelSelection.modelName, { reasoningEnabled: !isOff, reasoningBudget: newVal })
				}}
			/>
			<span className='text-brain-fg-3 text-xs pointer-events-none'>{isReasoningEnabled ? `${value} tokens` : 'Thinking disabled'}</span>
		</div>
	}
	...
	if (reasoningBudgetSlider?.type === 'effort_slider') {
		...
		const currentEffort = brainSettingsState.optionsOfModelSelection[featureName][modelSelection.providerName]?.[modelSelection.modelName]?.reasoningEffort ?? defaultVal
		const valueIfOff = -1
		...
		return <div className='flex items-center gap-x-2'>
			...
			<BrainSlider
				...
				onChange={(newVal) => {
					const isOff = canTurnOffReasoning && newVal === valueIfOff
					brainSettingsService.setOptionsOfModelSelection(featureName, modelSelection.providerName, modelSelection.modelName, { reasoningEnabled: !isOff, reasoningEffort: values[newVal] ?? undefined })
				}}
			/>
			<span className='text-brain-fg-3 text-xs pointer-events-none'>{isReasoningEnabled ? `${currentEffortCapitalized}` : 'Thinking disabled'}</span>
		</div>
	}
...
const ChatModeDropdown = ({ className }: { className: string }) => {
	...
	const onChangeOption = useCallback((newVal: ChatMode) => {
		brainSettingsService.setGlobalSetting('chatMode', newVal)
...
export const BrainChatArea: React.FC<BrainChatAreaProps> = ({
	children,
	onSubmit,
	onAbort,
	onClose,
	...
	return (
		<div
			...
			className={`
				...
				border border-brain-border-3 focus-within:border-brain-border-1 hover:border-brain-border-1
			...
			<div className="relative w-full">
				...
				{onClose && (
					<div className='absolute -top-1 -right-1 cursor-pointer z-1'>
						<IconX
							size={12}
							className="stroke-[2] opacity-80 text-brain-fg-3 hover:brightness-95"
							onClick={onClose}
						/>
					</div>
				)}
				...
				{showModelDropdown && (
					<div className='flex flex-col gap-y-1'>
						...
						<div className='flex items-center flex-wrap gap-x-2 gap-y-1 text-nowrap '>
							{featureName === 'Chat' && <ChatModeDropdown className='text-xs text-brain-fg-3 bg-brain-bg-1 border border-brain-border-2 rounded py-0.5 px-1' />}
							<ModelDropdown featureName={featureName} className='text-xs text-brain-fg-3 bg-brain-bg-1 rounded' />
						</div>
					</div>
				)}
...
export const SelectedFiles = (
	...
	const accessor = useAccessor()
	const commandService = accessor.get('ICommandService')
	const modelReferenceService = accessor.get('IBrainModelService')
					...
					>
						...
						<div
							className={`
								...
								text-xs text-nowrap
								border rounded-sm
								${isThisSelectionProspective ? 'bg-brain-bg-1 text-brain-fg-3 opacity-80' : 'bg-brain-bg-1 hover:brightness-95 text-brain-fg-1'}
								${isThisSelectionProspective
									? 'border-brain-border-2'
									: 'border-brain-border-1'
								}
								hover:border-brain-border-1
							...
							onClick={() => {
								...
								if (isThisSelectionProspective) { // add prospective selection to selections
									...
									brainOpenFileFn(selection.uri, accessor);
									...
									brainOpenFileFn(selection.uri, accessor, selection.range);
								}
						...
						>
							...
							{selection.type === 'File' && selection.state.wasAddedAsCurrentFile && messageIdx === undefined && currentURI?.fsPath === selection.uri.fsPath ?
								<span className={`text-[8px] 'brain-opacity-60 text-brain-fg-4`}>
									{`(Current File)`}
								</span>
								: null
							}
...
const ToolHeaderWrapper = ({
	...
	const desc1HTML = <span
		className={`text-brain-fg-4 text-xs italic truncate ml-2
	...
	return (<div className=''>
		<div className={`w-full border border-brain-border-3 rounded px-2 py-1 bg-brain-bg-3 overflow-hidden ${className}`}>
			{/* header */}
			<div className={`select-none flex items-center min-h-[24px]`}>
				<div className={`flex items-center w-full gap-x-2 overflow-hidden justify-between ${isRejected ? 'line-through' : ''}`}>
					...
					>
						...
						>
							{isDropdown && (<ChevronRight
								className={`
								text-brain-fg-3 mr-0.5 h-4 w-4 flex-shrink-0 transition-transform duration-100 ease-[cubic-bezier(0.4,0,0.2,1)]
								${isExpanded ? 'rotate-90' : ''}
							`}
							/>)}
							<span className="text-brain-fg-3 flex-shrink-0">{title}</span>
					...
					<div className="flex items-center gap-x-2 flex-shrink-0">
						...
						{info && <CircleEllipsis
							className='ml-2 text-brain-fg-4 opacity-60 flex-shrink-0'
							size={14}
							data-tooltip-id='brain-tooltip'
						...
						{isError && <AlertTriangle
							className='text-brain-warning opacity-90 flex-shrink-0'
							size={14}
							data-tooltip-id='brain-tooltip'
						...
						/>}
						{isRejected && <Ban
							className='text-brain-fg-4 opacity-90 flex-shrink-0'
							size={14}
							data-tooltip-id='brain-tooltip'
							data-tooltip-content={'Canceled'}
							data-tooltip-place='top'
						/>}
						{desc2 && <span className="text-brain-fg-4 text-xs" onClick={desc2OnClick}>
							{desc2}
						</span>}
						{numResults !== undefined && (
							<span className="text-brain-fg-4 text-xs ml-auto mr-1">
			...
			{<div
				className={`overflow-hidden transition-all duration-200 ease-in-out ${isExpanded ? 'opacity-100 py-1' : 'max-h-0 opacity-0'}
					text-brain-fg-4 rounded-sm overflow-x-auto
				  `}
			//    bg-black bg-opacity-10 border border-brain-border-4 border-opacity-50
			>
				{children}
			</div>}
		</div>
...
const EditTool = ({ toolMessage, threadId, messageIdx, content }: Parameters<ResultWrapper<'edit_file' | 'rewrite_file'>>[0] & { content: string }) => {
	...
	const desc1OnClick = () => brainOpenFileFn(params.uri, accessor)
	const componentParams: ToolHeaderParams = { title, desc1, desc1OnClick, desc1Info, isError, icon, isRejected, }


	const editToolType = toolMessage.name === 'edit_file' ? 'diff' : 'rewrite'
	if (toolMessage.type === 'running_now' || toolMessage.type === 'tool_request') {
		componentParams.children = <ToolChildrenWrapper className='bg-brain-bg-3'>
			<EditToolChildren
				uri={params.uri}
				code={content}
				type={editToolType}
		...
		<div>
			<div className="w-full">
				...
				>
					{isDropdown && (
						<ChevronRight
							className={`text-brain-fg-3 mr-0.5 h-4 w-4 flex-shrink-0 transition-transform duration-100 ease-[cubic-bezier(0.4,0,0.2,1)] ${isOpen ? 'rotate-90' : ''}`}
						/>
					)}
					<div className="flex items-center w-full overflow-hidden">
						<span className="text-brain-fg-3">{title}</span>
					</div>
				</div>
				{/* children */}
				{<div
					className={`overflow-hidden transition-all duration-200 ease-in-out ${isOpen ? 'opacity-100' : 'max-h-0 opacity-0'} text-brain-fg-4`}
				>
					{children}
				</div>}
			</div>
...
const UserMessageComponent = ({ chatMessage, messageIdx, isCheckpointGhost, currCheckpointIdx, _scrollToBottom }: { chatMessage: ChatMessage & { role: 'user' }, messageIdx: number, currCheckpointIdx: number | undefined, isCheckpointGhost: boolean, _scrollToBottom: (() => void) | null }) => {
	...
	if (mode === 'display') {
		...
		chatbubbleContents = <BrainChatArea
			featureName='Chat'
			onSubmit={onSubmit}
			onAbort={onAbort}
			isStreaming={false}
	...
	>
		<div
            ...
            ${mode === 'edit' ? ''
					: mode === 'display' ? 'p-2 flex flex-col bg-brain-bg-1 text-brain-fg-1 overflow-x-auto cursor-pointer' : ''
				}
        `}
		...
		>
			<EditSymbol
				...
				className={`
                    cursor-pointer
                    p-[2px]
                    bg-brain-bg-1 border border-brain-border-1 rounded-md
...
export const ListableToolItem = ({ name, onClick, isSmall, className, showDot }: { name: React.ReactNode, onClick?: () => void, isSmall?: boolean, className?: string, showDot?: boolean }) => {
	...
	>
		...
		<div className={`${isSmall ? 'italic text-brain-fg-4 flex items-center' : ''}`}>{name}</div>
	</div>
}
...
const EditToolChildren = ({ uri, code, type }: { uri: URI | undefined, code: string, type: 'diff' | 'rewrite' }) => {
	...
	const content = type === 'diff' ?
		<BrainDiffEditor uri={uri} searchReplaceBlocks={code} />
...
const LintErrorChildren = ({ lintErrors }: { lintErrors: LintErrorItem[] }) => {
	return <div className="text-xs text-brain-fg-4 opacity-80 border-l-2 border-brain-warning px-2 py-0.5 flex flex-col gap-0.5 overflow-x-auto whitespace-nowrap">
...
const BottomChildren = ({ children, title }: { children: React.ReactNode, title: string }) => {
	...
	return (
		<div className="w-full px-2 mt-0.5">
			...
			>
				<ChevronRight
					className={`mr-1 h-3 w-3 flex-shrink-0 transition-transform duration-100 text-brain-fg-4 group-hover:text-brain-fg-3 ${isOpen ? 'rotate-90' : ''}`}
				/>
				<span className="font-medium text-brain-fg-4 group-hover:text-brain-fg-3 text-xs">{title}</span>
			</div>
			<div
				className={`overflow-hidden transition-all duration-200 ease-in-out ${isOpen ? 'opacity-100' : 'max-h-0 opacity-0'} text-xs pl-4`}
			>
				<div className="overflow-x-auto text-brain-fg-4 opacity-90 border-l-2 border-brain-warning px-2 py-0.5">
					{children}
				</div>
			</div>
		</div>
...
const InvalidTool = ({ toolName, message, mcpServerName }: { toolName: ToolName, message: string, mcpServerName: string | undefined }) => {
	...
	componentParams.children = <ToolChildrenWrapper>
		<CodeChildren className='bg-brain-bg-3'>
			{message}
		</CodeChildren>
	</ToolChildrenWrapper>
	return <ToolHeaderWrapper {...componentParams} />
...
const builtinToolNameToComponent: { [T in BuiltinToolName]: { resultWrapper: ResultWrapper<T>, } } = {
	'read_file': {
		resultWrapper: ({ toolMessage }) => {
			...
			if (toolMessage.type === 'success') {
				const { result } = toolMessage
				componentParams.onClick = () => { brainOpenFileFn(params.uri, accessor, range) }
				...
				componentParams.children = !result.children || (result.children.length ?? 0) === 0 ? undefined
					: <ToolChildrenWrapper>
						{result.children.map((child, i) => (<ListableToolItem key={i}
							...
							onClick={() => {
								brainOpenFileFn(child.uri, accessor)
				...
				componentParams.children = result.uris.length === 0 ? undefined
					: <ToolChildrenWrapper>
						{result.uris.map((uri, i) => (<ListableToolItem key={i}
							...
							className='w-full overflow-auto'
							onClick={() => { brainOpenFileFn(uri, accessor) }}
						/>))}
						{result.hasNextPage &&
	...
	'read_lint_errors': {
		resultWrapper: ({ toolMessage }) => {
			...
			if (toolMessage.type === 'success') {
				const { result } = toolMessage
				componentParams.onClick = () => { brainOpenFileFn(params.uri, accessor) }
	...
	'create_file_or_folder': {
		resultWrapper: ({ toolMessage }) => {
			...
			if (toolMessage.type === 'success') {
				...
				const { result } = toolMessage
				if (params) { componentParams.onClick = () => { brainOpenFileFn(params.uri, accessor) } }
				componentParams.bottomChildren = <BottomChildren title='Error'>
					<CodeChildren>
						{result}
					</CodeChildren>
	...
	const fileDetailsContent = <div className="px-2 gap-1 w-full overflow-y-auto">
		{sortedCommandBarURIs.map((uri, i) => {
			...
			const fileNameHTML = <div
				className="flex items-center gap-1.5 text-brain-fg-3 hover:brightness-125 transition-all duration-200 cursor-pointer"
				onClick={() => brainOpenFileFn(uri, accessor)}
			>
				{/* <FileIcon size={14} className="text-brain-fg-3" /> */}
				<span className="text-brain-fg-3">{basename}</span>
			</div>
			...
			const detailsContent = <div className='flex px-4'>
				<span className="text-brain-fg-3 opacity-80">{numDiffs} diff{numDiffs !== 1 ? 's' : ''}</span>
			</div>
		...
		<>
			...
			<div className='px-2'>
				<div
					className={`
						select-none
						flex w-full rounded-t-lg bg-brain-bg-3
						text-brain-fg-3 text-xs text-nowrap
...
const EditToolSoFar = ({ toolCallSoFar, }: { toolCallSoFar: RawToolCallObj }) => {
	...
	const desc1OnClick = () => { uri && brainOpenFileFn(uri, accessor) }
...
export const SidebarChat = () => {
	...
	const keybindingString = accessor.get('IKeybindingService').lookupKeybinding(BRAIN_CTRL_L_ACTION_ID)?.getLabel()
	...
	>
		...
		{latestError === undefined ? null :
			<div className='px-2 my-1'>
				...
				<WarningBox className='text-sm my-2 mx-4' onClick={() => { commandService.executeCommand(BRAIN_OPEN_SETTINGS_ACTION_ID) }} text='Open settings' />
			</div>
		}
	...
	const inputChatArea = <BrainChatArea
		featureName='Chat'
		onSubmit={() => onSubmit()}
		onAbort={onAbort}
	...
	const initiallySuggestedPromptsHTML = <div className='flex flex-col gap-2 w-full text-nowrap text-brain-fg-3 select-none'>
		{[
			'Summarize my codebase',
			'How do types work in Rust?',
			'Create a .brainrules file for me'
		].map((text, index) => (
			<div
				key={index}
	...
	>
		...
		{Object.keys(chatThreadsState.allThreads).length > 1 ? // show if there are threads
			<ErrorBoundary>
				<div className='pt-8 mb-2 text-brain-fg-3 text-root select-none pointer-events-none'>Previous Threads</div>
				<PastThreadsList />
			</ErrorBoundary>
			:
			<ErrorBoundary>
				<div className='pt-8 mb-2 text-brain-fg-3 text-root select-none pointer-events-none'>Suggestions</div>
