...
import { FeatureName, featureNames, isFeatureNameDisabled, ModelSelection, modelSelectionsEqual, ProviderName, providerNames, SettingsOfProvider } from '../../../../../../../workbench/contrib/brain/common/brainSettingsTypes.js'
import { useSettingsState, useRefreshModelState, useAccessor } from '../util/services.js'
import { _BrainSelectBox, BrainCustomDropdownBox } from '../util/inputs.js'
import { SelectBox } from '../../../../../../../base/browser/ui/selectBox/selectBox.js'
import { IconWarning } from '../sidebar-tsx/SidebarChat.js'
import { BRAIN_OPEN_SETTINGS_ACTION_ID, BRAIN_TOGGLE_SETTINGS_ACTION_ID } from '../../../brainSettingsPane.js'
import { modelFilterOfFeatureName, ModelOption } from '../../../../../../../workbench/contrib/brain/common/brainSettingsService.js'
...
const ModelSelectBox = ({ options, featureName, className }: { options: ModelOption[], featureName: FeatureName, className: string }) => {
	...
	const selection = brainSettingsService.state.modelSelectionOfFeature[featureName]
	const selectedOption = selection ? brainSettingsService.state._modelOptions.find(v => modelSelectionsEqual(v.selection, selection))! : options[0]

	const onChangeOption = useCallback((newOption: ModelOption) => {
		brainSettingsService.setModelSelectionOfFeature(featureName, newOption.selection)
	}, [brainSettingsService, featureName])
...
export const ModelDropdown = ({ featureName, className }: { featureName: FeatureName, className: string }) => {
	...
	const openSettings = () => { commandService.executeCommand(BRAIN_OPEN_SETTINGS_ACTION_ID); };
