...
import { BrainFileSnapshot } from '../common/editCodeServiceTypes.js';
...
class ChatThreadService extends Disposable implements IChatThreadService {
	...
	constructor(
		...
		@IBrainModelService private readonly _voidModelService: IBrainModelService,
		@ILLMMessageService private readonly _llmMessageService: ILLMMessageService,
		@IToolsService private readonly _toolsService: IToolsService,
		@IBrainSettingsService private readonly _settingsService: IBrainSettingsService,
	...
	private _setState(state: Partial<ThreadsState>, doNotRefreshMountInfo?: boolean) {
		...
		if (streamState?.isRunning === undefined && !streamState?.error) {
			...
			// if awaiting user but stream state doesn't indicate it (happens if Restart 404Brain)
			if (lastMessage && lastMessage.role === 'tool' && lastMessage.type === 'tool_request')
				this._setStreamState(threadId, { isRunning: 'awaiting_user', })

			// if running now but stream state doesn't indicate it (happens if Restart 404Brain), cancel that last tool
	...
	private _swapOutLatestStreamingToolWithResult = (threadId: string, tool: ChatMessage & { role: 'tool' }) => {
		...
		// find THIS tool's own placeholder tile (its tool_request / running_now) by unique id.
		// id-matching instead of "last tool message" is what keeps concurrent (parallel) tool calls
		// from replacing each other's tiles. replacing in place keeps history in exact execution order.
		for (let i = messages.length - 1; i >= 0; i -= 1) {
			const m = messages[i]
			if (m.role === 'checkpoint') continue
			if (m.role === 'tool' && m.type === 'invalid_params') continue // terminal tile, never swap it
			if (m.role === 'tool' && (m as any).id === (tool as any).id) {
				this._editMessageInThread(threadId, i, tool)
				return true
			}
			if (m.role !== 'tool') break // stop at the first non-tool, non-checkpoint message from the end
		}
		return false
	}
	...
	private async _runChatAgent({
		...
		while (shouldSendAnotherMessage) {
			...
			while (shouldRetryLLM) {
				...
				// call the tools if there are any (a single response may batch multiple, opencode-style)
				if (toolCalls?.length) {
					const mcpTools = this._mcpService.getMCPTools()

					// if ANY call in the batch needs interactive approval, fall back to the sequential
					// single-approval flow so the existing approve/reject UX stays intact
					const requiresApproval = toolCalls.some(tc => {
						const approvalType = isABuiltinToolName(tc.name) ? approvalTypeOfBuiltinToolName[tc.name] : 'MCP tools'
						return !!approvalType && !this._settingsService.state.globalSettings.autoApprove[approvalType]
					})

					if (requiresApproval) {
						// run one by one, stop at the first call that waits for the user
						...
						shouldSendAnotherMessage = isRunningWhenEnd !== 'awaiting_user'
					}
					else {
						// all auto-approved -> fire them in PARALLEL, results come back together
						const results = await Promise.allSettled(toolCalls.map(tc => {
							const mcpTool = mcpTools?.find(t => t.name === tc.name)
						...
						// if the user aborted mid-batch, do NOT keep the agent loop alive
						if (results.some(r => r.status === 'fulfilled' && r.value?.interrupted)) {
							this._setStreamState(threadId, undefined)
							return
						}
	...
	private _getCheckpointInfo = (checkpointMessage: ChatMessage & { role: 'checkpoint' }, fsPath: string, opts: { includeUserModifiedChanges: boolean }) => {
		const brainFileSnapshot = checkpointMessage.brainFileSnapshotOfURI ? checkpointMessage.brainFileSnapshotOfURI[fsPath] ?? null : null
		if (!opts.includeUserModifiedChanges) { return { brainFileSnapshot, } }

		const userModifiedBrainFileSnapshot = fsPath in checkpointMessage.userModifications.brainFileSnapshotOfURI ? checkpointMessage.userModifications.brainFileSnapshotOfURI[fsPath] ?? null : null
		return { brainFileSnapshot: userModifiedBrainFileSnapshot ?? brainFileSnapshot, }
	}
	...
	private _computeNewCheckpointInfo({ threadId }: { threadId: string }) {
		...
		const brainFileSnapshotOfURI: { [fsPath: string]: BrainFileSnapshot | undefined } = {}
		...
		for (const fsPath in lastIdxOfURI ?? {}) {
			...
			if (!res) continue
			const { brainFileSnapshot: oldBrainFileSnapshot } = res
			...
			const brainFileSnapshot = this._editCodeService.getBrainFileSnapshot(URI.file(fsPath))
			if (oldBrainFileSnapshot === brainFileSnapshot) continue
			brainFileSnapshotOfURI[fsPath] = brainFileSnapshot
		}
		...
		return { brainFileSnapshotOfURI }
	}


	private _addUserCheckpoint({ threadId }: { threadId: string }) {
		const { brainFileSnapshotOfURI } = this._computeNewCheckpointInfo({ threadId }) ?? {}
		this._addCheckpoint(threadId, {
			role: 'checkpoint',
			type: 'user_edit',
			brainFileSnapshotOfURI: brainFileSnapshotOfURI ?? {},
			userModifications: { brainFileSnapshotOfURI: {}, },
		})
	}
	...
	private _addToolEditCheckpoint({ threadId, uri, }: { threadId: string, uri: URI }) {
		...
		const diffAreasSnapshot = this._editCodeService.getBrainFileSnapshot(uri)
		this._addCheckpoint(threadId, {
			role: 'checkpoint',
			type: 'tool_edit',
			brainFileSnapshotOfURI: { [uri.fsPath]: diffAreasSnapshot },
			userModifications: { brainFileSnapshotOfURI: {} },
		})
	}
	...
	private _getCheckpointsBetween({ threadId, loIdx, hiIdx }: { threadId: string, loIdx: number, hiIdx: number }) {
		...
		for (let i = loIdx; i <= hiIdx; i += 1) {
			...
			for (const fsPath in message.brainFileSnapshotOfURI) { // do not include userModified.beforeStrOfURI here, jumping should not include those changes
	...
	private _addUserModificationsToCurrCheckpoint({ threadId }: { threadId: string }) {
		...
		this._editMessageInThread(threadId, checkpointIdx, {
			...checkpoint,
			userModifications: { brainFileSnapshotOfURI: brainFileSnapshotOfURI ?? {}, },
		})
	}
		...
		if (toIdx < fromIdx) {
			...
			for (const fsPath in lastIdxOfURI) {
				...
				for (const k of idxes()) {
					...
					if (!res) continue
					const { brainFileSnapshot } = res
					if (!brainFileSnapshot) continue
					this._editCodeService.restoreBrainFileSnapshot(URI.file(fsPath), brainFileSnapshot)
					break
				}
			}
		}
