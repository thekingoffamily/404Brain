...
import { mountBrainOnboarding } from './react/out/brain-onboarding/index.js'
...
export class OnboardingContribution extends Disposable implements IWorkbenchContribution {
	static readonly ID = 'workbench.contrib.brainOnboarding';
	...
	private initialize(): void {
		...
		if (workbench) {
			...
			const onboardingContainer = h('div.brain-onboarding-container').root;
			workbench.appendChild(onboardingContainer);
			this.instantiationService.invokeFunction((accessor: ServicesAccessor) => {
				const result = mountBrainOnboarding(onboardingContainer, accessor);
