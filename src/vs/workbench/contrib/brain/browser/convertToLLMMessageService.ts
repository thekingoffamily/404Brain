...
class ConvertToLLMMessageService extends Disposable implements IConvertToLLMMessageService {
	...
	// Read .brainrules files from workspace folders
	private _getBrainRulesFileContents(): string {
		try {
			const workspaceFolders = this.workspaceContextService.getWorkspace().folders;
			let brainRules = '';
			for (const folder of workspaceFolders) {
				const uri = URI.joinPath(folder.uri, '.brainrules')
				const { model } = this.brainModelService.getModel(uri)
				if (!model) continue
				brainRules += model.getValue(EndOfLinePreference.LF) + '\n\n';
			}
			return brainRules.trim();
		}
		catch (e) {
			return ''
		}
	...
	prepareLLMSimpleMessages: IConvertToLLMMessageService['prepareLLMSimpleMessages'] = ({ simpleMessages, systemMessage, modelSelection, featureName }) => {
		...
		const { overridesOfModel } = this.brainSettingsService.state
		...
		const modelSelectionOptions = this.brainSettingsService.state.optionsOfModelSelection[featureName][modelSelection.providerName]?.[modelSelection.modelName]
	...
	prepareLLMChatMessages: IConvertToLLMMessageService['prepareLLMChatMessages'] = async ({ chatMessages, chatMode, modelSelection }) => {
		...
		const { disableSystemMessage } = this.brainSettingsService.state.globalSettings;
		const fullSystemMessage = await this._generateChatMessagesSystemMessage(chatMode, specialToolFormat)
		const systemMessage = disableSystemMessage ? '' : fullSystemMessage;

		const modelSelectionOptions = this.brainSettingsService.state.optionsOfModelSelection['Chat'][modelSelection.providerName]?.[modelSelection.modelName]
