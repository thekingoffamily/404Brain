...
import { BRAIN_VIEW_CONTAINER_ID, BRAIN_VIEW_ID } from './sidebarPane.js';
import { IMetricsService } from '../common/metricsService.js';
import { ICommandService } from '../../../../platform/commands/common/commands.js';
import { BRAIN_TOGGLE_SETTINGS_ACTION_ID } from './brainSettingsPane.js';
...
const BRAIN_OPEN_SIDEBAR_ACTION_ID = 'brain.sidebar.open'
registerAction2(class extends Action2 {
	constructor() {
		super({ id: BRAIN_OPEN_SIDEBAR_ACTION_ID, title: localize2('brainOpenSidebar', '404Brain: Open Sidebar'), f1: true });
	}
	async run(accessor: ServicesAccessor): Promise<void> {
		const viewsService = accessor.get(IViewsService)
		const chatThreadsService = accessor.get(IChatThreadService)
		viewsService.openViewContainer(BRAIN_VIEW_CONTAINER_ID)
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: BRAIN_CTRL_L_ACTION_ID,
			f1: true,
			title: localize2('brainCmdL', '404Brain: Add Selection to Chat'),
			keybinding: {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		// open panel
		const wasAlreadyOpen = viewsService.isViewContainerVisible(BRAIN_VIEW_CONTAINER_ID)
		if (!wasAlreadyOpen) {
			await commandService.executeCommand(BRAIN_OPEN_SIDEBAR_ACTION_ID)
		}
...
const BRAIN_CMD_SHIFT_L_ACTION_ID = 'brain.cmdShiftL'
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: BRAIN_CMD_SHIFT_L_ACTION_ID,
			title: 'New Chat',
			keybinding: {
			...
			icon: { id: 'add' },
			menu: [{ id: MenuId.ViewTitle, group: 'navigation', when: ContextKeyExpr.equals('view', BRAIN_VIEW_ID), }],
		});
	}
	async run(accessor: ServicesAccessor): Promise<void> {
...
registerAction2(class extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		commandService.executeCommand(BRAIN_CMD_SHIFT_L_ACTION_ID)
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: 'brain.settingsAction',
			title: `404Brain Settings`,
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		const commandService = accessor.get(ICommandService)
		commandService.executeCommand(BRAIN_TOGGLE_SETTINGS_ACTION_ID)
	}
})
