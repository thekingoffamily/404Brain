...
export class BrainMainUpdateService extends Disposable implements IBrainUpdateService {
	...
	async check(explicit: boolean): Promise<BrainCheckUpdateRespose> {
		...
		if (this._updateService.state.type === StateType.Ready) {
			// Update is ready
			return { message: 'Restart 404Brain to update!', action: 'restart' } as const
		}
	...
	private async _manualCheckGHTagIfDisabled(explicit: boolean): Promise<BrainCheckUpdateRespose> {
		try {
			const response = await fetch('https://api.github.com/repos/thekingoffamily/404Brain/releases/latest');
			...
			if (explicit) {
				if (response.ok) {
					if (!isUpToDate) {
						message = 'A new version of 404Brain is available! Please reinstall (auto-updates are disabled on this OS) - it only takes a second!'
						action = 'reinstall'
					}
					else {
