...
export class MetricsMainService extends Disposable implements IMetricsService {
	...
	// helper - looks like this is stored in a .vscdb file in ~/Library/Application Support/404Brain
	...
	private get oldId() {
		...
		const newKey = 'brain.app.oldMachineId'
		...
		const oldValue = this._appStorage.get('brain.machineId', StorageScope.APPLICATION) ?? 'NULL' // the old way of getting the key
		...
		// private get oldId() {
		// 	return this._memoStorage('brain.app.oldMachineId', StorageTarget.MACHINE, 'NULL')
		// }
	}
	...
	private get distinctId() {
		...
		return this._memoStorage('brain.app.machineId', StorageTarget.MACHINE, setValIfNotExist)
	}

	// just to see if there are ever multiple machineIDs per userID (instead of this, we should just track by the user's email)
	private get userId() {
		return this._memoStorage('brain.app.userMachineId', StorageTarget.USER)
	}
	...
	async initialize() {
		...
		const { commit, version, brainVersion, release, quality } = this._productService
		...
		console.log('User is opted out of basic 404Brain metrics?', didOptOut)
		if (didOptOut) {
			this.client.optOut()
		}
		else {
		...
		console.log('404Brain posthog metrics info:', JSON.stringify(identifyMessage, null, 2))
	}
