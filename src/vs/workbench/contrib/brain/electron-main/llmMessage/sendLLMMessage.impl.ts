...
import { ChatMode, displayInfoOfProviderName, ModelSelectionOptions, OverridesOfModel, ProviderName, SettingsOfProvider } from '../../common/brainSettingsTypes.js';
	...
	openai.chat.completions
		...
		.then(async response => {
			...
			if (!fullTextSoFar && !fullReasoningSoFar && !toolName) {
				...
				const toolCallObj = toolCall ? { toolCall, toolCalls } : {}
...
const sendAnthropicChat = async ({ messages, providerName, onText, onFinalMessage, onError, settingsOfProvider, modelSelectionOptions, overridesOfModel, modelName: modelName_, _setAborter, separateSystemMessage, chatMode, mcpTools }: SendChatParams_Internal) => {
	...
	stream.on('finalMessage', (response) => {
		...
		const toolCalls = tools.map(t => rawToolCallObjOfAnthropicParams(t)).filter((t): t is NonNullable<typeof t> => t !== null)
