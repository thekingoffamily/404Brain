...
/* eslint-disable */ // Brain
import { BRAIN_CTRL_K_ACTION_ID, BRAIN_CTRL_L_ACTION_ID } from '../../../contrib/brain/browser/actionIDs.js';
...
export class EditorGroupWatermark extends Disposable {
	...
	private render(): void {
		...
		this.clear();
		const brainIconBox = append(this.shortcuts, $('.watermark-box'));
		...
		const update = async () => {
			...
			// Brain - if the workbench is empty, show open
			if (this.contextService.getWorkbenchState() === WorkbenchState.EMPTY) {
				...
				brainIconBox.appendChild(buttonContainer);

				// Open a folder
				const openFolderButton = h('button')
				openFolderButton.root.classList.add('brain-openfolder-button')
				...
				openSSHButton.root.classList.add('brain-openssh-button')
				...
				if (recentlyOpened.length !== 0) {
					...
					brainIconBox.append(
						...recentlyOpened.map((w, i) => {
							...
							const linkSpan = $('span');
							linkSpan.classList.add('brain-link')
				...
				// show them Brain keybindings
				const keys = this.keybindingService.lookupKeybinding(BRAIN_CTRL_L_ACTION_ID);
				const dl = append(brainIconBox, $('dl'));
				...
				const keys2 = this.keybindingService.lookupKeybinding(BRAIN_CTRL_K_ACTION_ID);
				const dl2 = append(brainIconBox, $('dl'));
