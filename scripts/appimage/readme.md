...
stable-linux.yml in the separate `brain-builder/` repo,
which runs a GitHub Action that builds the AppImage you see on our website.


# Brain AppImage Creation Script

This script automates the process of creating an AppImage for the  Brain Editor using Docker. It works on macOS and Linux platforms.
## Requirements
...
5.  **Result:**
    ...
    After the script completes, it will generate an AppImage named `Brain-x86_64.AppImage` (or similar, depending on your architecture) in the current directory.
...
*   **AppImage Creation:**
    *   Creates the `BrainApp.AppDir` structure.
    *   Copies binaries, resources, and the `.desktop` entry.
    *   Copies `brain.desktop` and `brain.png`.
